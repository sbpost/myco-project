curl_company_ids <- function() {
  # This function collects company records from the
  # https://www.myco.dica.gov.mm/ site. The function collects all the unique
  # company ID's on the site. This is done through the search API. 
  ...
  # once. 
  # I therefor run a search for each lower-case letter (a-z) and for 
...
get_company_profiles <- function(company_ids) {
  # This function uses the unique company IDs to grab company profiles
  # from the MyCo website using their CorpProfile API. There are quite
  # a lot of records, so the the HTTP requests are performed async.
